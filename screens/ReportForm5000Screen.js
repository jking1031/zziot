...
import { StyleSheet, View, Text, ScrollView, TextInput, TouchableOpacity, Alert, Image, ActivityIndicator } from 'react-native';
...
const ReportFormScreen = ({ route }) => {
  ...
  const checkExistingReport = async () => {
    ...
    try {
      const response = await axios.get(`https://nodered.jzz77.cn:9003/api/reports5000/check?date=${formData.date}&operator=${formData.operator}`);
    ...
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.form}>
        ...
        <View style={styles.card}>
          ...
          {renderFormField('曝气系统运行状态', 'aeration_system_status', '描述曝气系统运行情况')}
          {renderFormField('反洗系统运行状态', 'backwash_system_status', '描述反洗系统运行情况')}
          {renderFormField('进水泵系统运行状态', 'inlet_pump_status', '描述进水泵系统运行情况')}
          {renderFormField('磁混凝运行状态', 'magnetic_mixing_status', '描述磁混凝系统运行情况')}
          {renderFormField('中间水箱及清水箱运行状态', 'water_tank_status', '描述水箱运行情况')}
          {renderFormField('剩余污泥排放情况', 'sludge_discharge_status', '描述污泥排放情况')}
          {renderFormField('其他设备设施运行情况', 'other_equipment_status', '描述其他设备运行情况')}
        </View>

        <View style={styles.card}>
          <Text style={[styles.sectionTitle, { color: colors.text }]}>药剂投加情况</Text>
          {renderFormField('絮凝剂投加量 (L)', 'flocculant_dosage', '请输入絮凝剂投加量', 'numeric', true)}
          {renderFormField('磁粉投加量 (kg)', 'magnetic_powder_dosage', '请输入磁粉投加量', 'numeric', true)}
          {renderFormField('各药剂剩余存量', 'chemical_inventory', '请填写各类药剂的剩余存量')}
        </View>
